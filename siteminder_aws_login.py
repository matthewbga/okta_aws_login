...
# sid_cache_file: The file where the SiteMinder sid is stored.
...
def get_user_creds():
    ...
    # Next check to see if the SITEMINDER_USERNAME env var is set
    elif os.environ.get("SITEMINDER_USERNAME") is not None:
        username = os.environ.get("SITEMINDER_USERNAME")
    # Otherwise just ask the user
    else:
        username = input("Username: ")
    # Set prompt to include the user name, since username could be set
    # via SITEMINDER_USERNAME env and user might not remember.
...
def siteminder_mfa_login(password_login_response,app):
    ...
    headers_dict['X-SiteMinder-Xsrftoken'] = xsrfToken
...
def update_config_file(siteminder_aws_login_config_file):
    ...
    print("cache_sid determines if the session id from SiteMinder should be saved "
            "to a local file. If enabled it allows for new tokens to be "
            "retrieved without a login to SiteMinder for the lifetime of the "
...
def main():
    ...
    if os.path.isfile(siteminder_aws_login_config_file):
        config = configparser.ConfigParser()
        config.read(siteminder_aws_login_config_file)
